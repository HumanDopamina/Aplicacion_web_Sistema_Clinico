...
export default function PatientDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-6xl">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(250px,0.8fr)]"><div className="space-y-5"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-4"><h2 className="font-serif text-xl font-semibold text-slate-900">Información personal</h2>{canEdit ? <button type="button" aria-label="Editar información personal" onClick={() => setEditing(true)} className="text-xs font-semibold text-blue-700 hover:underline">Editar</button> : null}</div><dl className="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2"><DataItem label="Nombre completo" value={patient.full_name} /><DataItem label="Fecha de nacimiento" value={dateFormatter.format(new Date(`${patient.date_of_birth}T00:00:00Z`))} /><DataItem label="Género" value={genderLabels[patient.gender]} /><DataItem label="Lugar de nacimiento" value={patient.birth_place} /><DataItem label="Cédula" value={patient.national_id} /><DataItem label="Dirección" value={patient.address} /><DataItem label="Teléfono" value={patient.phone} /><DataItem label="Correo electrónico" value={patient.email} /></dl></section><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-serif text-xl font-semibold text-slate-900">Resumen clínico</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-slate-100 bg-slate-50 p-4"><small className="text-slate-400">Alergias</small><p className="mt-1 text-sm font-semibold text-slate-700">Sin información registrada</p></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-4"><small className="text-slate-400">Tipo de sangre</small><p className="mt-1 text-sm font-semibold text-slate-700">Sin información registrada</p></div></div></section></div><aside className="space-y-5"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-4"><h2 className="font-serif text-lg font-semibold text-slate-900">Contacto de emergencia</h2>{canEdit ? <button type="button" aria-label="Editar contacto de emergencia" onClick={() => setEditing(true)} className="text-xs font-semibold text-blue-700 hover:underline">Editar</button> : null}</div><dl className="mt-5 grid gap-4"><DataItem label="Nombre" value={patient.emergency_contact_name} /><DataItem label="Parentesco" value={patient.emergency_relationship} /><DataItem label="Teléfono" value={patient.emergency_phone} /></dl></section><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-serif text-lg font-semibold text-slate-900">Antecedentes</h2><div className="mt-3"><EmptyClinicalItem label="Antecedentes familiares" /><EmptyClinicalItem label="Antecedentes personales" /><EmptyClinicalItem label="Procedimientos" /></div></section></aside></div>
    {editing ? <PatientFormModal accessToken={accessToken} patient={patient} onClose={() => setEditing(false)} onSaved={(savedPatient) => { setPatient(savedPatient); setEditing(false) }} /> : null}
  </div>
}
