...
const editablePatient = (patient) => Object.fromEntries(
  Object.keys(emptyPatient).map((field) => [field, patient?.[field] ?? emptyPatient[field]]),
)
...
export default function PatientFormModal({ accessToken, onClose, onSaved, patient = null }) {
  const isEditing = Boolean(patient)
  const [form, setForm] = useState(() => editablePatient(patient))
  ...
  const submit = async (event) => {
    ...
    try {
      const saved = isEditing
        ? await updatePatient(accessToken, patient.id, form)
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/45 p-4" role="presentation">
    <section role="dialog" aria-modal="true" aria-labelledby="patient-form-title" className="my-4 max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      <div className="flex items-start justify-between px-6 pt-6 sm:px-8">
        <div>
          <h2 id="patient-form-title" className="font-serif text-2xl font-semibold text-slate-900">{isEditing ? 'Editar paciente' : 'Añadir nuevo paciente'}</h2>
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4 px-6 pb-6 sm:grid-cols-2 sm:px-8">
        <Field label="Código"><input className={`${fieldClass} bg-slate-50 text-slate-500`} value={patient?.code || 'Se genera automáticamente'} readOnly /></Field>
        ...
        <div className="-mx-6 -mb-6 mt-2 flex justify-end gap-2 border-t border-slate-100 px-6 py-5 sm:col-span-2 sm:-mx-8 sm:px-8">
          ...
          <button disabled={saving} type="submit" aria-label={isEditing ? 'Guardar cambios' : 'Añadir paciente'} className="rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-60">{saving ? 'Guardando…' : isEditing ? 'Guardar cambios' : '＋ Añadir paciente'}</button>
        </div>
      </form>
    </section>
  </div>
