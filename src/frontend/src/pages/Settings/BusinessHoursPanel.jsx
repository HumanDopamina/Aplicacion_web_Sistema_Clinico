...
export default function BusinessHoursPanel({ accessToken }) {
  ...
  useEffect(() => {
    Promise.all([getBusinessHours(accessToken), listClosures(accessToken)])
      .then(([hours, items]) => { setDays(hours.days); setClosures(items) })
  ...
  const save = async () => {
    ...
    try { setDays((await updateBusinessHours(accessToken, { days })).days); setMessage('Horarios guardados correctamente.') }
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="hours-title">
    <header className="flex items-start justify-between gap-4 border-b border-slate-100 p-5"><div><h2 id="hours-title" className="font-serif text-2xl font-semibold text-slate-900">Horarios de atención</h2><p className="mt-1 text-xs text-slate-500">Configura jornadas, pausas y cierres de la clínica.</p></div><button onClick={save} disabled={saving} className="rounded-xl bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Guardando…' : 'Guardar cambios'}</button></header>
