...
export default function PatientRecordPage({ isNew = false }) {
  ...
  return <form onSubmit={submit} className="mx-auto w-full max-w-6xl">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-2">
      ...
      <SectionCard title="Datos personales"><DataGrid form={form} canModify={canModify} onChange={update} items={[
      ...
      <SectionCard title="Motivo de consulta"><RecordValue label="Motivo de consulta" value={recordSource.chief_complaint} field="chief_complaint" type="textarea" form={form} canModify={canModify} onChange={update} /></SectionCard>
      <SectionCard title="Historia de la enfermedad actual"><RecordValue label="Historia de la enfermedad actual" value={recordSource.present_illness_history} field="present_illness_history" type="textarea" form={form} canModify={canModify} onChange={update} /></SectionCard>
      <SectionCard title="Interrogatorio por aparatos y sistemas" wide><DataGrid form={form} canModify={canModify} onChange={update} items={[
      ...
      <SectionCard title="Examen físico" wide><DataGrid form={form} canModify={canModify} onChange={update} columns="sm:grid-cols-2 lg:grid-cols-4" items={[
      ...
      ].map(([label, field]) => ({ label, field, value: recordSource[field], type: numericFields.has(field) ? 'number' : 'text' }))} /><div className="mt-6"><DataGrid form={form} canModify={canModify} onChange={update} items={[
