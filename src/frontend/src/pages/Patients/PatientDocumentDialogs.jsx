  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-0 backdrop-blur-[2px] sm:p-5">
    <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="document-upload-title" className="h-full w-full overflow-y-auto bg-white shadow-2xl sm:h-auto sm:max-h-[calc(100dvh-2.5rem)] sm:max-w-2xl sm:rounded-3xl">
      <header className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-200 bg-white px-5 py-5 sm:px-7">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-700">Archivo clínico</p><h2 id="document-upload-title" className="mt-1 font-serif text-3xl font-semibold text-slate-950">Adjuntar documentos</h2><p className="mt-1 text-sm text-slate-500">Los metadatos se aplicarán a todo el lote.</p></div>
...
export function DocumentDetailDialog({ accessToken, document, patientActive, canDelete, patientId, onClose, onDeleted }) {
  ...
  const remove = async () => {
    ...
    setError('')
    try {
      await deletePatientDocument(accessToken, patientId, document.id)
      await onDeleted()
      onClose()
    } catch (requestError) {
      setError(requestError.message)
