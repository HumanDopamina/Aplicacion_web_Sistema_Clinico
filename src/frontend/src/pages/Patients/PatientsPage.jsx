...
export default function PatientsPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLoading(true)
      setError('')
      listPatients(accessToken, search, page > 1 ? page : undefined)
        .then((data) => {
          if (!active) return
  ...
  return <div className="mx-auto w-full max-w-6xl">
    ...
    <label className="relative mt-8 block rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><span className="sr-only">Buscar pacientes</span><span aria-hidden="true" className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400">⌕</span><input type="search" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Buscar por nombre, cédula, teléfono o correo…" className="w-full rounded-lg border border-slate-200 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label>
